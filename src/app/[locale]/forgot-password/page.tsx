...
export default function ForgotPasswordPage() {
  ...
  return (
    <div className="bg-gradient-mesh min-h-screen flex items-center justify-center font-body-md text-on-surface p-6 sm:p-12 relative overflow-hidden bg-[#f8f9ff]">
      ...
      <div className="fixed top-6 end-6 z-20">
      ...
      <main className="w-full max-w-[480px] z-10">
        ...
        <div className="flex flex-col items-center mb-12">
          ...
          <p className="font-label-caps text-xs text-outline font-bold uppercase tracking-widest">{tCommon("tagline")}</p>
        </div>
          ...
          <div className="relative z-10">
            {submitted ? (
              <div className="text-center space-y-6 py-4">
                ...
                <div>
                  <h2 className="font-title-md text-xl font-bold text-on-background mb-2">{t("successTitle")}</h2>
              ...
              <>
                <div className="mb-8 text-center">
                  <h2 className="font-title-md text-xl font-bold text-on-background mb-2">{t("resetTitle")}</h2>
                  <p className="font-body-sm text-sm text-on-surface-variant max-w-[280px] mx-auto">
                    {t("resetSubtitle")}
                  </p>
                </div>
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <label className="font-label-caps text-xs text-on-surface-variant font-bold block ms-1" htmlFor="email">
                      {t("emailLabel")}
                    </label>
                    <div className="relative group">
                      <div className="absolute inset-y-0 start-0 ps-4 flex items-center pointer-events-none text-outline group-focus-within:text-primary transition-colors">
                        <Mail className="w-5 h-5" />
                      </div>
                      <input
                        className="w-full ps-12 pe-4 py-4 bg-white/50 border border-outline-variant/40 rounded-2xl font-body-md text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary focus:bg-white transition-all outline-none text-on-surface"
                        id="email"
                        name="email"
        ...
        <div className="mt-8 flex items-center justify-center gap-2">
          ...
          <span className="font-label-caps text-xs text-outline font-bold uppercase">{t("bankGradeSecurity")}</span>
        </div>
      </main>

      {/* Footer */}
      <footer className="fixed bottom-0 left-0 w-full p-4 text-center hidden md:block">
        <p className="font-body-sm text-xs text-outline font-bold">{tCommon("rightsReserved")}</p>
      </footer>
    </div>
  );
}
