  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start relative min-h-[calc(100vh-10rem)]">
      ...
      <div className="col-span-12 lg:col-span-8 flex flex-col justify-between h-full bg-white/40 glass-card beveled-edge rounded-3xl p-6 min-h-[600px]">
        {/* Messages List */}
        <div className="flex-1 overflow-y-auto space-y-6 mb-6 pr-2 max-h-[450px]">
          {messages.map((msg, i) => {
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              onClick={() => handleSuggestedPrompt("Predict next month revenue")}
            ...
            <button
              onClick={() => handleSuggestedPrompt("Show risky expenses")}
              className="px-4 py-2 rounded-full border border-primary/20 bg-white/50 backdrop-blur-sm text-primary text-xs font-bold hover:bg-primary hover:text-white transition-all cursor-pointer"
            >
              Show risky expenses
            </button>
            <button
              onClick={() => handleSuggestedPrompt("Analyze cash flow")}
              className="px-4 py-2 rounded-full border border-primary/20 bg-white/50 backdrop-blur-sm text-primary text-xs font-bold hover:bg-primary hover:text-white transition-all cursor-pointer"
            >
              Analyze cash flow
            </button>
          ...
          <div className="bg-white/80 border border-outline-variant/30 p-2 rounded-[2rem] shadow-xl flex items-end gap-2 group focus-within:ring-2 focus-within:ring-primary/30 transition-all">
            ...
            <button
              ...
              className="p-4 bg-primary text-white rounded-full mb-1 mr-1 active:scale-90 transition-transform shadow-lg shadow-primary/30 flex items-center justify-center cursor-pointer"
            >
              <Send className="w-4 h-4" />
            </button>
          </div>
          <p className="text-center text-[10px] text-on-surface-variant/40 font-bold uppercase tracking-wider">
            Compta AI may provide inaccurate financial data. Please verify important insights.
          </p>
        </div>
      </div>
      ...
      <div className="col-span-12 lg:col-span-4 space-y-6">
        <div className="glass-card beveled-edge p-6 rounded-3xl bg-white/70 flex flex-col justify-between min-h-[480px]">
          <div className="space-y-4">
            <h3 className="text-xs uppercase font-bold text-on-surface-variant tracking-wider">Active Insights</h3>
            {/* Anomaly Card */}
            <div className="glass-card p-4 rounded-2xl border-l-4 border-error/50 bg-white/55">
              <div className="flex items-start gap-3">
                <div className="bg-error/10 p-2 rounded-lg text-error">
                  <AlertTriangle className="w-5 h-5" />
                </div>
                <div className="flex-grow">
                  <h4 className="font-bold text-sm text-on-surface">Anomaly Detected</h4>
                  <p className="text-xs text-on-surface-variant mt-1">
                    Found duplicate invoice for Amazon Web Services (1,240.00 DZD)
                  </p>
            ...
            <div className="glass-card p-4 rounded-2xl border-l-4 border-tertiary bg-white/55">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-grow">
                  ...
                  <p className="text-xs text-on-surface-variant mt-1">
                    Switch to annual billing on Slack to save 450 DZD/year
                  </p>
                  <button className="mt-2 text-primary font-bold text-xs hover:underline cursor-pointer">
                    Automate Task
                  </button>
          ...
          <div className="space-y-4 pt-6">
            <h3 className="text-xs uppercase font-bold text-on-surface-variant tracking-wider">Contextual Tools</h3>
            <div className="grid grid-cols-1 gap-2">
              <button className="flex items-center gap-3 w-full p-4 glass-card rounded-xl hover:bg-white transition-all group bg-white/40 cursor-pointer text-left">
                <Upload className="w-5 h-5 text-primary group-hover:scale-110 transition-transform shrink-0" />
                <span className="text-sm font-bold text-on-surface">Upload Invoices</span>
              </button>
              <button className="flex items-center gap-3 w-full p-4 glass-card rounded-xl hover:bg-white transition-all group bg-white/40 cursor-pointer text-left">
                <Share className="w-5 h-5 text-primary group-hover:scale-110 transition-transform shrink-0" />
                <span className="text-sm font-bold text-on-surface">Export Insights</span>
              </button>
              <button className="flex items-center gap-3 w-full p-4 glass-card rounded-xl hover:bg-white transition-all group bg-white/40 cursor-pointer text-left">
                <History className="w-5 h-5 text-primary group-hover:scale-110 transition-transform shrink-0" />
                <span className="text-sm font-bold text-on-surface">Analysis History</span>
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="relative group overflow-hidden rounded-3xl h-44 shadow-xl">
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-slate-900/80 to-transparent p-6 flex flex-col justify-end">
            <p className="text-white font-bold text-base leading-tight">Unlock Predictive AI</p>
            <p className="text-white/70 text-xs mt-1">Upgrade to Enterprise Plan</p>
          </div>
        </div>
      </div>
    </div>
