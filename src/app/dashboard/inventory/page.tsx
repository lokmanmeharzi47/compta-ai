  ...
  return (
    <div className="space-y-8 pb-16 relative">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
        <div>
          <h2 className="font-headline-lg text-3xl font-bold text-on-surface">Inventory Management</h2>
          <p className="text-body-md text-on-surface-variant">Real-time oversight of your asset ecosystem.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="glass-card beveled-edge p-6 rounded-3xl relative overflow-hidden bg-white/70">
          ...
          <div className="mt-4 flex items-center gap-1 text-primary text-xs font-bold">
            ...
            <span>Next arrival: Tomorrow 09:00 AM</span>
          </div>
        </div>
      </div>
          ...
          {items.some((item) => item.status === "LOW STOCK") && (
            <div className="bg-amber-500/10 border border-amber-500/20 rounded-3xl p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between backdrop-blur-sm gap-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h4 className="font-bold text-lg text-on-background">Low Stock Alert: Essential Components</h4>
                  <p className="text-sm text-on-surface-variant">
                    Some core items are below the safety threshold. Production may be impacted.
                  </p>
                </div>
              </div>
              <button
              ...
              {items.map((item, idx) => (
                ...
                >
                  <div className="relative h-48 w-full rounded-2xl overflow-hidden mb-4 bg-slate-100">
                    ...
                    <div
                      className={`absolute top-2 right-2 px-2 py-1 rounded-lg text-[9px] font-black tracking-widest ${
                  ...
                  <div className="px-3 pb-3">
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        ...
                        <p className="font-mono-data text-xs text-outline font-bold">SKU: {item.sku}</p>
                      </div>
            ...
            <div className="glass-card beveled-edge rounded-3xl overflow-hidden bg-white/40">
              <table className="w-full text-left">
                <thead className="bg-[#eff4ff]/60 border-b border-outline-variant/20">
                  <tr>
                    <th className="px-6 py-4 text-xs font-bold text-on-surface-variant uppercase">Product</th>
                    <th className="px-6 py-4 text-xs font-bold text-on-surface-variant uppercase">SKU</th>
                    <th className="px-6 py-4 text-xs font-bold text-on-surface-variant uppercase">Price</th>
                    <th className="px-6 py-4 text-xs font-bold text-on-surface-variant uppercase">Quantity</th>
          ...
          <div className="glass-card beveled-edge p-6 rounded-3xl h-full flex flex-col justify-between bg-white/70">
            <div>
              ...
              <p className="text-xs text-on-surface-variant mb-6">
                Efficiency ratio for the Top 5 most active products this quarter.
              </p>
            ...
            <div className="mt-8 p-4 bg-primary/5 rounded-2xl border border-primary/10 flex items-center gap-3">
              ...
              <p className="text-xs text-on-surface-variant leading-tight">
                AI Suggestion: Reallocate 15% of storage from Zenith Drone to Nexus Core to optimize logistics.
              </p>
            </div>
          </div>
        </div>
      ...
      <button
        onClick={handleAddNewItem}
        className="fixed bottom-8 right-8 h-14 w-14 bg-primary text-white rounded-full shadow-2xl shadow-primary/40 flex items-center justify-center hover:scale-105 active:scale-95 transition-all z-50 cursor-pointer"
      >
